...
export default function BottomNav() {
  const { activeTab, setActiveTab, setShowContact, setSegment, segment } = useApp()
  ...
  const handleTap = (id) => {
    ...
    const sectionMap = { home: 'hero', startup: 'startup', individual: 'individual', team: 'team' }
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-40 md:hidden">
      <div className={`${isStartup ? 'glass-dark' : 'glass-light'} border-t ${
        isStartup ? 'border-white/5' : 'border-gray-200/50'
      }`}>
          ...
          {tabs.map(({ id, label, icon: Icon, highlight }) => {
            ...
            return (
              ...
              >
                ...
                {highlight ? (
                  <div className={`w-10 h-10 -mt-3 rounded-full flex items-center justify-center shadow-lg ${
                    isStartup ? 'bg-gold-500 shadow-gold-500/30' : 'bg-royal-500 shadow-royal-500/30'
                  }`}>
                ...
                <span className={`text-[10px] mt-0.5 truncate transition-colors ${
                  highlight
                    ? isStartup ? 'text-gold-400 font-semibold' : 'text-royal-500 font-semibold'
                    : isActive
                      ? isStartup ? 'text-gold-400 font-semibold' : 'text-royal-500 font-semibold'
                      : isStartup ? 'text-navy-500' : 'text-gray-400'
                }`}>
                  {label}
                </span>
              </motion.button>
